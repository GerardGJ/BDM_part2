...
def spark_sql(sqlcontext,query):
    url = "jdbc:sqlite:./FormattedZone"
    return sqlcontext.read.format('jdbc').options(url=url, dbtable=query, driver='org.sqlite.JDBC').load()
#def spark_mongo(spark,query):
    #return spark.read.format('com.mongodb.spark.sql.DefaultSource').options(uri= "mongodb://10.4.41.47:27017/FormattedZone.table_markets", drivers='org.mongodb.spark:mongo-spark-connector_2.11:2.3.2' ).load()
...
sqlcontext = spark_sql_init()

query="(select * from markets) as t"
df=spark_sql(sqlcontext,query)